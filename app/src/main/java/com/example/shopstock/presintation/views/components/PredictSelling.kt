...
fun getThePredictArrow( predict: Int ,lastDay: Int):Int{
    if (predict>lastDay){
        return R.drawable.ic_trending_up
    }else if (predict<lastDay){
        return R.drawable.ic_trending_down
    }
    return R.drawable.ic_trending_staight
}

@Composable
fun PredictSelling(item: ItemEntity) {
    Row(
        verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(
            RawSpacedBy)
    ) {
        Image(
            painter = painterResource(getThePredictArrow(item.predictSelling ,item.soldLast7Days.last())),
            contentDescription = stringResource(R.string.predict_chart_image),
            modifier = Modifier.size(CardIconSize)
        )
        Text(
            text = stringResource(R.string.predict_sales_tomorrow), style = TextStyle(
                color = ItemFontColor,
        ...
        Text(
            text = item.predictSelling.toString(), style = TextStyle(
